...
import type { MediaProcessingResult, FileValidationResult } from '../types/content.types';
...
export class MediaService {
  ...
  static async processImage(
    ...
    try {
      ...
      // Upload to S3
      const url = await uploadFileToS3(processedBuffer, fileName, file.mimetype);
  ...
  static async processVideo(
    ...
    try {
      ...
      // Upload video to S3
      const videoUrl = await uploadFileToS3(file.buffer, videoFileName, file.mimetype);
      ...
      throw new Error('Failed to process video', { cause: error });
    }
  }
  ...
  static async processGif(
    ...
    try {
      ...
      const url = await uploadFileToS3(file.buffer, fileName, file.mimetype);
      ...
      throw new Error('Failed to process GIF', { cause: error });
    }
  }
  ...
  static async generateVideoThumbnail(_videoBuffer: Buffer): Promise<Buffer> {
  ...
  static async getMediaMetadata(
    file: Express.Multer.File,
    type: MediaType
  ): Promise<{ width?: number; height?: number; format?: string; duration?: number | null }> {
    try {
      switch (type) {
        case MediaType.IMAGE:
        case MediaType.GIF: {
        ...
        case MediaType.VIDEO:
          ...
          return {
            // Placeholder — no dimensions available without FFmpeg
            width: undefined,
            height: undefined,
            duration: undefined,
          };
