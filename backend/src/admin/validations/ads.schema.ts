...
export const createAdSchema = z.object({
  ...
  targetRegion: z.array(z.string().length(2, 'Region must be a 2-letter ISO country code')).min(1, 'At least one target region is required'),
  ...
  frequencyCap: z.number().int('Frequency cap must be an integer').positive('Frequency cap must be positive').max(100, 'Frequency cap must not exceed 100'),
...
export const updateAdSchema = z.object({
  ...
  targetRegion: z.array(z.string().length(2, 'Region must be a 2-letter ISO country code')).min(1, 'At least one target region is required').optional(),
  targetGender: z.enum(['male', 'female', 'all']).optional(),
  category: z.string().optional(),
  cpm: z.number().positive('CPM must be a positive number').max(10000, 'CPM must not exceed 10000').optional(),
  frequencyCap: z.number().int('Frequency cap must be an integer').positive('Frequency cap must be positive').max(100, 'Frequency cap must not exceed 100').optional(),
